...
**Pipelines and jobs are declarations, authored as object models; only steps are discovered by convention.** An `IPipeline` (`Name` = the `ritten.json` identifier, `Label` = the printed human name) exposes stable job instances through its `Jobs` property. A job is a class extending `Job<TSettings>`, overriding `Name`, `StepTypes` (an ordered list of step `Type`s), and as needed `ConfigureServices(services, settings)` and `ValidateSettings(settings)` — the latter takes a `SettingsValidator<TSettings>` offering `Require(s => s.Build.Project)` (the error's `ritten.json` key derives from the property chain) and `RequireEnvironment("VAR")` (dry runs warn instead of failing). Validation and registration are deferred until settings exist, so a job's shape cannot depend on any project's configuration, and the whole registry→pipeline→job→step tree is data at startup.

`Program.cs` registers pipeline instances in a `PipelineRegistry` and maps each CLI job name to `PipelineHost.Run(registry, request, ct)` (`JobRequest` carries job/log-level/flags). The host runs in stages: validate the whole registered model (duplicate names, malformed steps, and the structural job rules — every registered job, every run), resolve `ritten.json` (walking up from the cwd), select the pipeline by the required `"pipeline"` key, select the job *before* settings parse (a typo'd command needs no valid config to diagnose), strict-deserialize settings against the job's settings record (camelCase, unknown keys rejected; every settings record extends `PipelineSettings`), then `PipelineHostBuilder.Build(job, settings)` assembles the one chosen job — services delegate, steps, requirements, dry-run decoration, pipeline-registered rules, and a DI container (`Microsoft.Extensions.DependencyInjection`, with `ValidateOnBuild`). `DefaultPipelineRunner` executes the steps in order.

**Steps** are minimal-API-style classes, read by reflection into the `Step` model (`Contracts/Step.cs`, `Step.Describe`) — one object carrying both the facts rules and reporters consume (`Name`/`Kind`/`Produces`/`Requires`) and the internal machinery to invoke the step; a `Step` built from its facts alone (as rule tests do) cannot be invoked:
