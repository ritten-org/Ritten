...
**Runtimes are detected, never declared.** `Core/Runtimes/` holds the model: hosts register `PipelineRuntime` candidates in a `RuntimeRegistry` (mirroring `PipelineRegistry`; validated with the rest of the model), and `PipelineHostBuilder.Build` detects the active one first. A runtime *matches* on its own marker variables only and *claims* the variables it owns — compatibility surfaces included — so precedence between matching candidates derives from claims (a runtime that impersonates another's variables outranks it; unresolvable overlap fails loudly), and the claimed names are removed from the `PipelineEnvironment` view that everything downstream, settings validation included, reads. No match falls back to the engine-owned `LocalRuntime`, which registers nothing. The active runtime contributes runtime-dependent services — report sinks, the PR comment, `RunContext`, an ambient credential offered via `PostConfigure` — while GitHub-as-*destination* (`AddGitHubClient`: `IGitHubClient`, `IReleaseService`, explicit `GH_TOKEN`) stays declared by jobs, because releasing to GitHub is independent of running on it.
