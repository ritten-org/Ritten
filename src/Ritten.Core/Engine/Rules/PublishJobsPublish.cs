...
/// <summary>
/// Requires a job that says it publishes to actually contain a publish step, and one that says
/// it only checks to contain none.
/// </summary>
public sealed class PublishJobsPublish : IJobRule
{
    /// <inheritdoc />
    public IEnumerable<Error> Check(IJob job)
    {
        var publishes = job.Steps.Where(step => step.Kind == StepKind.Publish).ToList();
        switch (job.Kind)
        {
            case JobKind.Publish when publishes.Count == 0:
