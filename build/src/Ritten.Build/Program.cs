...
build.SetAction((_, cancellationToken) => PipelineHost.Run<DotNetPackageBuild>(cancellationToken));

var verify = new Command("verify", "Compiles and tests, without any release validation.");
verify.SetAction((_, cancellationToken) => PipelineHost.Run<DotNetPackageVerify>(cancellationToken));

var deploy = new Command("deploy", "Validates, packs, tags, creates the GitHub release, and publishes to NuGet.");
deploy.SetAction((_, cancellationToken) => PipelineHost.Run<DotNetPackageDeploy>(cancellationToken));
