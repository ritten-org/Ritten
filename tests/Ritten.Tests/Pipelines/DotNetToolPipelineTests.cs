...
public class DotNetToolPipelineTests
{
    ...
    public void Deploy_BuildsWithoutAnyEnvironment()
    {
        ...
        var result = Build("deploy", Complete, environment: PipelineRunBuilderHelpers.Empty);

        result.IsSuccess.ShouldBeTrue();
        result.Value.Dispose();
    }

    private static Result<PipelineRun> Build(
        string job,
        string settings,
        ...
        bool dryRun = false)
    {
        ...
        var builder = PipelineRunBuilderHelpers.Create(pipeline.Label, environment, dryRun, settings: settings);
